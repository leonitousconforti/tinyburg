import { Config, DateTime, Duration, Effect, Encoding, Layer, Option, Redacted, Schema } from "effect";
...
import type { NimblebitGame } from "@tinyburg/nimblebit-sdk/NimblebitGame";
...
export const accessTokenFor = Effect.fnUntraced(function* ({ session, user }: { session: Session; user: User }) {
    ...
    const SESSION_SCOPE = "openid profile tinytower tinytowerclassic treasury";
...
export const ApiLive = HttpApiBuilder.layer(Api).pipe(
    ...
    // its own. A custom host (the dev stack's fake Nimblebit) takes over
    // when NIMBLEBIT_HOST is set; production never sets it.
    Layer.provide(
        Layer.unwrap(
            Effect.map(
                Config.option(Config.string("NIMBLEBIT_HOST")),
                Option.match({
                    onNone: () => NimblebitAuth.layerDirectConfig(),
                    onSome: (host) =>
