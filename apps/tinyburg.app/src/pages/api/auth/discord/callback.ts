 ...
 *
 * Handles the OAuth callback from Discord, exchanges the code for tokens,
...
export const GET: APIRoute = async ({ redirect, request }) => {
    ...
    try {
        // Get OAuth config
        const config = await Effect.runPromise(DiscordOAuthConfig);
        ...
        // Get display name (prefer global_name, fallback to username)
