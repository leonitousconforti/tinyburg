/**
 * The TinyTower Classic api, proxied exactly as TinyTower's is: the same
 * handlers shape, against the Classic endpoints, under the Classic scopes the
 * authorization middleware reads off each endpoint.
 */
...
import { Endpoints as ClassicEndpoints } from "@tinyburg/tinytower-classic-sdk";
...
/** @internal */
class Client extends Context.Service<Client>()("ClassicClient", {
    make: HttpApiClient.make(ClassicEndpoints.Api, {
...
/** @internal */
const Api = ClassicEndpoints.Api.middleware(Authorization).middleware(AuthProxyApiDecodeHash);

/**
 * @since 1.0.0
 * @category TinyTower Classic Routes
 */
export const ClassicRaffleLive = HttpApiBuilder.group(
    Api,
    "RaffleGroup",
 ...
 * @category TinyTower Classic Routes
 */
export const ClassicDeviceManagementLive = HttpApiBuilder.group(
    Api,
 ...
 * @category TinyTower Classic Routes
 */
export const ClassicSyncManagementLive = HttpApiBuilder.group(
    Api,
    "SyncManagementGroup",
    Effect.fnUntraced(function* (handlers) {
 ...
 * @category TinyTower Classic Routes
 */
export const ClassicSocialGroupLive = HttpApiBuilder.group(
    Api,
    "SocialGroup",
    Effect.fnUntraced(function* (handlers) {
 ...
 * @category TinyTower Classic Routes
 */
export const TinyTowerClassicApiLive = HttpApiBuilder.layer(Api).pipe(
    Layer.provide([ClassicRaffleLive, ClassicDeviceManagementLive, ClassicSyncManagementLive, ClassicSocialGroupLive]),
