...
import { Endpoints } from "@tinyburg/bitcity-sdk";

import { AuthProxyApiDecodeHash, AuthProxyApiDecodeHashLive } from "../middleware/20_tinytowerDecode.ts";

/** @internal */
class Client extends Context.Service<Client>()("BitCityClient", {
...
/**
 * @since 1.0.0
 * @category Bit City Routes
 */
export const BitCityDeviceManagementLive = HttpApiBuilder.group(
    Api,
 ...
 * @category Bit City Routes
 */
export const BitCitySyncManagementLive = HttpApiBuilder.group(
    Api,
    "SyncManagementGroup",
    Effect.fnUntraced(function* (handlers) {
 ...
 * @category Bit City Routes
 */
export const BitCitySocialLive = HttpApiBuilder.group(
    Api,
    "SocialGroup",
    Effect.fnUntraced(function* (handlers) {
 ...
 * @category Bit City Routes
 */
export const BitCityApiLive = HttpApiBuilder.layer(Api).pipe(
    Layer.provide([BitCityDeviceManagementLive, BitCitySyncManagementLive, BitCitySocialLive]),
    Layer.provide(AuthProxyApiDecodeHashLive),
    Layer.provide(
        NimblebitAuth.layerDirectConfig(
            Config.schema(NimblebitConfig.NimblebitAuthKeySchema, "BITCITY_NIMBLEBIT_AUTH_KEY")
        )
    ),
