 ...
 *
 * Both roads out of the treasury end at Nimblebit's sync service: the vault's
 * direct calls obviously, but also every trading-api call, because
 * tinyburg.app forwards them upstream without a gate of its own. So both go
 * through this one semaphore, and the treasury as a whole never exceeds the
 * house pace however many trades are in flight.
 *
 * It is also the reason the cluster runs as a single node: a distributed
 * escrow would need this bucket to be distributed too, which is real work in
 * exchange for parallelism the rate limit forbids us from using.
 */

import { Clock, Config, Context, Duration, Effect, Layer, Ref, Semaphore } from "effect";

export class NimblebitPacer extends Context.Service<NimblebitPacer>()("@tinyburg/treasury/services/NimblebitPacer", {
    make: Effect.gen(function* () {
        /**
         * Minimum gap between two calls. Conservative by default; a slow
         * trade is strictly better than getting the whole project blocked.
         */
