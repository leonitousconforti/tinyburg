/**
 * The scopes the Bit City api is guarded by, as a game-rooted tree, built with
 ...
 * @category Games
 */
export const BitCity = defineGame({
    name: "bitcity",
    description: "Bit City",
    read: { description: "Everything in Bit City that only looks" },
    write: { description: "Everything in Bit City that changes a player's game" },
    areas: {
        device: {
 ...
 * @category Areas
 */
export const { device: Device, social: Social } = BitCity.areas;

/**
 * @since 1.0.0
 * @category Tree
 */
export const Games: ReadonlyArray<Game> = [BitCity];
