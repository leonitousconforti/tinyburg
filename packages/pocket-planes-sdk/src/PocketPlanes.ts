 ...
 * All method preimages below are the Tiny Tower formula with the `pp` game code.
 *
...
export const device_newPlayer = Effect.gen(function* () {
    ...
    const hash = yield* nimblebitAuth.sign(`pp/${salt1}/${salt2}`);
...
export const device_playerDetails = Effect.fn("device_playerDetails")(function* ({
    ...
    const hash = yield* nimblebitAuth.sign(`pp/${playerId}/${salt}${Redacted.value(playerAuthKey)}`);
...
/**
 * Lists visits (bitizen/plane exchanges) waiting for the player. VERIFIED.
 *
 * @since 1.0.0
 * @category SDK
 */
...
/**
 * Lists gifts waiting for the player. VERIFIED.
 *
 * @since 1.0.0
 * @category SDK
 */
...
/**
 * Lists item requests waiting for the player (Pocket Planes specific;
 * `{ success, gifts }` with no `total`). VERIFIED.
 *
 * @since 1.0.0
 * @category SDK
 */
export const social_requestItems = Effect.fn("social_requestItems")(function* ({
    playerAuthKey,
    playerId,
}: Schema.Schema.Type<typeof NimblebitConfig.AuthenticatedPlayerSchema>) {
    const nimblebitAuth = yield* NimblebitAuth.NimblebitAuth;
