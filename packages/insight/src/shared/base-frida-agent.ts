...
export abstract class TinyTowerFridaAgent<T extends ITinyTowerFridaAgent> {
    ...
    private async _waitForDependencies<T extends DependencyLoader>(
        ...
        try {
            ...
            if (!justDependencies.every(([dep]) => !dep.isNull())) {
                ...
                console.log("Searching for any class dependencies that have static constructors...");
                for (const [dep, meta] of classDependencies) {
                    ...
                    if (meta?.callStaticConstructor && klass.hasStaticConstructor) {
                        console.log(`Initializing class ${klass.name} by calling its static constructor now`);
                        klass.initialize();
                    }
                }
            ...
            console.log("Success, all dependencies have been loaded!");
    ...
    protected sendToProgram(message: unknown, data?: ArrayBuffer | number[] | null | undefined): void {
        send(message, data);
    }
